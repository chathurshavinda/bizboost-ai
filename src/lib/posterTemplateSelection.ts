import { type PlanDayTheme, type PosterTemplateCategory, type PosterTemplateDefinition, POSTER_TEMPLATE_CATALOG, getPosterTemplateById, } from "@/src/lib/posterTemplateCatalog";
...
export function normalizePlanDayTheme(raw: string | undefined | null): PlanDayTheme {
    ...
    if (PLAN_DAY_THEMES.includes(t as PlanDayTheme))
        return t as PlanDayTheme;
    return "Promo";
}
...
function scoreTemplate(t: PosterTemplateDefinition, planDay: PlanDayTheme, businessTypeLower: string, categoryRank: (cat: PosterTemplateCategory) => number): number {
    let score = 0;
    ...
    for (const kw of t.businessTypeKeywords) {
        ...
        if (k && businessTypeLower.includes(k))
            score += 10;
    }
    return score;
}
export function pickPosterTemplateForContext(opts: {
    ...
    let pool = POSTER_TEMPLATE_CATALOG.filter((t) => t.dayThemes.length === 0 || t.dayThemes.includes(planDay));
    if (pool.length === 0)
        pool = [...POSTER_TEMPLATE_CATALOG];
...
export function pickPosterStyleForContext(dayTheme: string | undefined, dayNumber: number, businessType?: string): PosterStyle {
...
export function resolvePosterTemplateOrDefault(templateId: string | undefined | null, fallbackStyle: PosterStyle): {
    template: PosterTemplateDefinition;
    style: PosterStyle;
} {
    const t = getPosterTemplateById(templateId ?? "");
    if (t)
        return { template: t, style: t.layoutType };
    const general = getPosterTemplateById(DEFAULT_GENERAL_TEMPLATE_ID);
    if (general)
        return { template: general, style: general.layoutType };
    return {
        template: {
            id: "inline-fallback",
            name: "Fallback",
