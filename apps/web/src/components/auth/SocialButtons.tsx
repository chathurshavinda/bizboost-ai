...
export default function SocialButtons({
  ...
  const labelFor = (name: ProviderName) => {
    if (busy === name) return "Continuing...";
    if (name === "google") return mode === "signup" ? "Sign up with Google" : "Continue with Google";
    if (name === "github") return "Continue with GitHub";
    return "Continue with Facebook";
  };

  return (
    <div className="authRefProviderRow">
      <button
        type="button"
        onClick={() => handle("google")}
        disabled={disabled || busy !== null}
        className="authRefProviderButton"
      >
