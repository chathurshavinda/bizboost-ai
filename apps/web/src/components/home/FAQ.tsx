...
export default function FAQ() {
    ...
    return (<Reveal className="max-w-[1160px] mx-auto mt-10" delayMs={150}>
      <section id="faq" className="rounded-3xl border border-black/15 bg-white/60 backdrop-blur-2xl shadow-[0_24px_70px_rgba(0,0,0,0.10)] p-6 md:p-8">
        ...
        <div className="mt-6 space-y-3">
          {faqs.map((item, index) => {
            ...
            return (<article key={item.question} className="rounded-2xl border border-black/10 bg-gradient-to-b from-white/90 to-neutral-100/80 shadow-[0_10px_28px_rgba(0,0,0,0.07)]">
                <button type="button" onClick={() => setOpenIndex(isOpen ? null : index)} className="w-full text-left px-5 py-4 flex items-center justify-between gap-4" aria-expanded={isOpen}>
