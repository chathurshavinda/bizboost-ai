...
export default function SideMenu({ open, onClose, onRequestLogout }: SideMenuProps) {
  ...
  return (
    <>
      <div
        className={`bbSideBackdrop ${open ? "bbSideBackdrop--open" : ""}`}
        aria-hidden
        onClick={onClose}
      />
      <aside
        className={`bbSideMenuPanel ${open ? "bbSideMenuPanel--open" : ""}`}
        role="dialog"
        aria-modal="true"
        aria-label="BizBoost navigation"
      >
        ...
        <nav className="bbSideNavScroll">
          ...
          <SideItem
            ...
            onClick={() => go("/onboarding/business-details")}
          ...
          <SideItem
            icon={<FaGem size={15} />}
            label="Select plan"
            onClick={() => go("/select-plan")}
            active={pathname === "/select-plan"}
          />
          <SideItem
            icon={<FaClipboardList size={15} />}
            label="Plan Builder"
            onClick={() => go("/marketing-plan")}
            active={pathname === "/marketing-plan"}
          />
          <SideItem
            icon={<FaEdit size={15} />}
            label="Biz Editor"
            onClick={() => go("/biz-editor")}
            active={pathname === "/biz-editor"}
          />
          <SideItem
            icon={<FaImage size={15} />}
            label="Poster preview"
            onClick={() => go("/poster-preview")}
            active={pathname === "/poster-preview"}
          />
          <SideItem
            icon={<FaCalendarDay size={15} />}
            label="Biz Calendar"
            onClick={() => go("/biz-calendar")}
            active={pathname === "/biz-calendar"}
          />
...
function SideItem({ icon, label, onClick, active = false, danger }: SideItemProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`bbSideItem ${active ? "bbSideItem--active" : ""} ${danger ? "bbSideItem--danger" : ""}`}
      role="menuitem"
    >
