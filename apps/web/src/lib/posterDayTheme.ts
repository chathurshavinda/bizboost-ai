...
/** Must match marketing plan generator `deriveDayTheme` values */
...
/** Internal marketing cycle theme keys from plan generator → user-facing poster day theme */
export function deriveDayThemeForPlan(internalTheme: string, weekend: boolean, festivalTrim: string): DayTheme {
  if (festivalTrim.trim()) return "Promo";
  if (weekend && (internalTheme === "promo_offer" || internalTheme === "growth_push")) return "WeekendSpecial";
...
/** Stable accent per business name for BizBoost posters */
...
/**
 * Chooses visual layout (`PosterStyle`) from catalog + plan day theme + optional business type + day rotation.
 */
export function pickPosterStyleForDay(
...
/** Initial poster fields from today's plan before / after AI refine */
export function buildPosterSeedFromPlan(args: {
  ...
  const headline =
    ...
    "YOUR OFFER TODAY";
  const subSource =
    (ma?.postBrief || ma?.contentBrief || "").trim() ||
    captionRest ||
    (args.businessGrowthAction || "").trim();
  const subheadline = squash(subSource, 100);
  const offerBadge =
    squash(ma?.offerDeadlineHint || "", 28) || squash(ma?.posterHint || args.posterHintFallback || "", 28);
