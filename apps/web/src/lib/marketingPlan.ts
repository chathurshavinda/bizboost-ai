...
export function buildLifecyclePlanDays(
  rawPlanDays: Array<Partial<MarketingPlanDay> & Record<string, unknown>>,
  durationDays: 7 | 14 | 30,
  startDateISO: string,
): MarketingPlanDay[] {
  ...
  return Array.from({ length: durationDays }, (_, index) => {
    ...
    return {
      ...
      marketingActivation:
        typeof raw.marketingActivation === "object" && raw.marketingActivation
          ? {
              channel: ["instagram", "facebook", "both"].includes(
                String((raw.marketingActivation as Record<string, unknown>).channel ?? "").toLowerCase(),
              )
                ? (String((raw.marketingActivation as Record<string, unknown>).channel ?? "").toLowerCase() as
                    | "instagram"
                    | "facebook"
                    | "both")
                : "both",
              formatPlan: Array.isArray((raw.marketingActivation as Record<string, unknown>).formatPlan)
                ? (((raw.marketingActivation as Record<string, unknown>).formatPlan as unknown[])
                    .map((s) => String(s))
                    .filter((s) => ["Reel", "Story", "FeedPost", "Carousel"].includes(s)) as Array<
              ...
              storyFrames: Array.isArray((raw.marketingActivation as Record<string, unknown>).storyFrames)
                ...
                : [],
              reelScript:
                typeof (raw.marketingActivation as Record<string, unknown>).reelScript === "object" &&
...
export function normalizePlanDocument(doc: Record<string, unknown> | null): MarketingPlanDocument | null {
  ...
  const planDays = buildLifecyclePlanDays(
