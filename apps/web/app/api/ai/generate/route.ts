...
function validateBody(body: unknown): {
    ...
    if (!SUPPORTED_TYPES.includes(typeRaw as GenerateType)) {
        return {
            ok: false,
            message: "type is required and must be 'marketing-plan', 'poster', 'social-post', 'poster-caption', 'final-post', or 'poster-design'",
        };
    }
    ...
    const isShortFormType = typeRaw === "social-post" ||
...
function buildPosterDesignPrompt(details: BusinessDetails): string {
    ...
    const hasActivationContext = !!details.activationPostBrief ||
...
function buildMarketingPlanPrompt(details: BusinessDetails): string {
    ...
    const fmt = (label: string, value: string | undefined) => value && value.trim() ? `- ${label}: ${value.trim()}` : "";
    const knownLines = [
