...
/** BSON-safe payload for NextResponse.json (avoids serialization errors on ObjectId / Date). */
...
export async function GET(request: Request) {
  try {
    ...
    if (!profile) {
      ...
      return NextResponse.json(
        { ok: false, error: "business_profile_not_found" },
        { status: 404 }
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await db.collection("business_profiles").updateOne(
      filter,
      {
        $set: {
          firebase_uid,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: "business_profile_delete_failed" },
      { status: 500 }
    );
  }
}
