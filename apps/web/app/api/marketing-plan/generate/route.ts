...
function missingRequiredProfileFields(profile: Record<string, unknown>, body: Record<string, unknown>): string[] {
  ...
  if (products.length === 0) missing.push("productsOrServices");
  if (!location) missing.push("location");
  if (!goals) missing.push("businessGoals");
...
function resolveProfileContext(
  ...
  const products = toStringArray(
    profile.productsOrServices ?? profile.products ?? profile.services ?? profile.items ?? "",
  );
  const inferredCategory = normalizeCategoryFromText(
    toCleanString(profile.businessType),
    products.join(" "),
    toCleanString(profile.industry),
  );
  const inferredBusinessType = toCleanString(
    profile.businessType,
    inferredCategory !== "generic" ? inferredCategory : "generic",
  );
...
function ensurePlanDataLength(planData: DayPlan[], planDays: number, context: TemplateContext, startDate: Date): DayPlan[] {
  if (Array.isArray(planData) && planData.length === planDays) return planData;
  const safe = Array.isArray(planData) ? planData.filter(Boolean) : [];
  if (safe.length === 0) return buildPlan(planDays, context, startDate);
...
function getSriLankaFestival(date: Date): string {
  ...
  if ((m === 4 && d >= 10 && d <= 16)) return "Avurudu";
  if (m === 5 && d >= 20 && d <= 28) return "Vesak";
  if (m === 6 && d >= 18 && d <= 25) return "Poson";
  if (m === 3 && d >= 8 && d <= 31) return "Ramadan";
  if (m === 10 && d >= 20 && d <= 31) return "Deepavali";
  if (m === 12 && d >= 20 && d <= 31) return "Christmas";
  return "";
}
...
async function generateNarrativePlan(input: NarrativeInput): Promise<string> {
  ...
  const isOverloaded = (m: string) =>
    /503|overload|service unavailable|high demand|rate limit|429|quota|temporarily/i.test(m);
  ...
  for (const modelName of modelCandidates) {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        console.error(
          `[marketing-plan/generate] narrative Gemini error on ${modelName} (attempt ${attempt + 1}):`,
          message,
        );
...
function uniqueTitle(title: string, usedTitles: Set<string>, dayNumber: number): string {
  ...
  if (!nextTitle) nextTitle = `Growth action day ${dayNumber}`;
...
function shouldTryAnotherVariant(
  theme: ThemeKey,
  ...
  const sameRecentAngle =
    memory.recentThemes.includes(theme) &&
...
function weekendNote(category: CategoryKey): string {
  if (category === "food") return "Weekend adaptation: use this as a weekend special, tasting event, or dine-in/takeaway push.";
...
function buildDayOutput(theme: ThemeKey, input: DayBuilderInput): DayBuilderOutput {
  ...
  const activationTemplate =
    activations[(input.dayNumber + input.variantIndex + input.weekIndex) % activations.length] ??
  ...
  const weekendHint =
    input.weekend && input.category === "food"
...
export async function POST(req: Request) {
  try {
    ...
    const planDays =
      normalizePlanDays(requestedDays) ??
    ...
    if (existingPlan && !forceNew) {
      ...
      const existingTemplateVersion =
        typeof existingPlan.templateVersion === "string" ? existingPlan.templateVersion : "";
    ...
    const { context, inferredBusinessType, planStartDate, requestedPlanStartDateISO } = resolveProfileContext(
    ...
    const lifecyclePlanDays = buildLifecyclePlanDays(
      (Array.isArray(planData) ? planData : []) as Array<Record<string, unknown>>,
      planDays,
      startDate,
    );
    ...
    if (forceNew) {
      await db.collection("marketing_plans").updateMany(
