...
type ApiErrorCode = "business_profile_not_found" | "plan_not_selected" | "generic" | "";
...
export default function GenerateMarketingPlanPage() {
  ...
  return (
    <div className="min-h-screen px-4 py-8 text-black sm:px-6 sm:py-10">
      <div className="mx-auto w-full max-w-6xl space-y-6 sm:space-y-8">
        <section className="rounded-3xl border border-black/15 bg-black/10 p-5 shadow-[0_16px_50px_rgba(0,0,0,0.08)] backdrop-blur-xl transition-all duration-300 sm:p-8">
          <div className="mb-4 flex items-center justify-between">
            <button
              type="button"
              onClick={() => router.push("/select-plan")}
              className="rounded-full border border-black/15 bg-white/60 px-3 py-1.5 text-xs font-medium text-black/80 transition hover:scale-[1.02] hover:bg-white/90"
            >
              Back
            </button>
          </div>
          ...
          <div className="space-y-5">
              ...
              <div className="mt-2 inline-flex rounded-full border border-black/15 bg-black/10 p-1">
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                    channel === "Instagram" ? "bg-black text-white shadow-sm" : "text-black/70 hover:bg-white/80"
                  }`}
                >
                  Instagram
                </button>
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                    channel === "Facebook" ? "bg-black text-white shadow-sm" : "text-black/70 hover:bg-white/80"
                  }`}
                >
                  Facebook
                </button>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., Increase sales / awareness"
                className="mt-2 w-full rounded-2xl border border-black/15 bg-white/70 px-4 py-3 text-sm text-black outline-none transition focus:border-black/40 focus:ring-2 focus:ring-black/10"
              />
            </div>
            ...
            <button
              ...
              onClick={generate}
              className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-black/20 bg-black px-5 py-3 text-sm font-semibold text-white transition hover:scale-[1.01] hover:bg-black/90 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
            >
              {loading ? (
                <>
        ...
        {errorContent && (
          <section className="rounded-2xl border border-black/15 bg-black/10 p-4 shadow-[0_12px_35px_rgba(0,0,0,0.08)] backdrop-blur-xl sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm font-medium text-black/85">{errorContent.title}</p>
              <button
                type="button"
                onClick={errorContent.onClick}
                className="inline-flex items-center justify-center rounded-xl border border-black/15 bg-white px-4 py-2 text-sm font-medium text-black transition hover:bg-black hover:text-white"
              >
        ...
        {plan.length > 0 && (
          <section className="rounded-3xl border border-black/15 bg-black/10 p-5 shadow-[0_18px_55px_rgba(0,0,0,0.08)] backdrop-blur-xl transition-all duration-300 sm:p-8">
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="text-xl font-semibold text-black sm:text-2xl">Your Plan ({planDays || plan.length} Days)</h2>
              <button
                type="button"
                onClick={downloadJson}
                className="rounded-xl border border-black/15 bg-white px-4 py-2 text-sm font-medium text-black transition hover:bg-black hover:text-white"
              >
                Download JSON
              </button>
            </div>
            ...
            <div className="space-y-3">
              {plan.map((d) => {
                ...
                return (
                  <article
                    key={d.day}
                    className="overflow-hidden rounded-2xl border border-black/10 bg-white/80 shadow-[0_8px_20px_rgba(0,0,0,0.05)] transition-all duration-300"
                  >
                    <button
                      type="button"
                      onClick={() => setOpenDay((prev) => (prev === d.day ? null : d.day))}
                      className="flex w-full items-center justify-between px-4 py-3 text-left transition hover:bg-black/5"
                    >
                    ...
                    {isOpen && (
                      <div className="space-y-4 border-t border-black/10 px-4 py-4 text-sm text-black/80 sm:px-5">
                        <div>
                        ...
                        <div>
                          <div className="mb-2 flex items-center justify-between gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => copyCaption(d.caption, d.day)}
                              className="rounded-lg border border-black/15 bg-white px-3 py-1 text-xs font-medium text-black transition hover:bg-black hover:text-white"
                            >
                        ...
                        <div>
                          ...
                          <div className="mt-2 flex flex-wrap gap-2">
                            {d.hashtags.map((tag) => (
                              <span
                                key={`${d.day}-${tag}`}
                                className="rounded-full border border-black/12 bg-white px-2.5 py-1 text-xs text-black/70"
                              >
                                {tag}
                              </span>
                            ))}
