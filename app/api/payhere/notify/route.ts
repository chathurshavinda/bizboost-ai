...
import { isPlanDays, mapStatusCode, planPeriodMs, verifyNotificationSignature, type PlanDays, } from "@/lib/payhere";
...
export async function POST(req: Request) {
    try {
        ...
        await db.collection("payhere_orders").updateOne({ order_id }, {
            $set: {
                payment_id,
                status,
                status_code,
        ...
        if (status === "success") {
            ...
            await subscriptions.updateOne({ firebase_uid }, {
                $set: {
                    firebase_uid,
                    planDays,
                    status: "active",
            ...
            await subscriptions.updateOne({ firebase_uid }, { $set: { status: "cancelled", cancelledAt: now, updatedAt: now } });
        }
        else if (status === "failed") {
            await subscriptions.updateOne({ firebase_uid }, { $set: { status: "failed", updatedAt: now } });
        }
        else if (status === "chargedback") {
            await subscriptions.updateOne({ firebase_uid }, { $set: { status: "chargedback", updatedAt: now } });
        }
        return new NextResponse("OK", { status: 200 });
    }
    catch (error: unknown) {
