...
export async function POST(req: Request) {
    if (!devConfirmEnabled()) {
        return NextResponse.json({ ok: false, error: "dev_confirm_disabled" }, { status: 403 });
    }
    try {
        ...
        if (!firebase_uid || !order_id) {
            return NextResponse.json({ ok: false, error: "firebase_uid and order_id are required" }, { status: 400 });
        }
        ...
        if (order.firebase_uid !== firebase_uid) {
            return NextResponse.json({ ok: false, error: "order_mismatch" }, { status: 403 });
        }
        const planDaysRaw = Number(order.planDays ?? 0);
        if (!isPlanDays(planDaysRaw)) {
            return NextResponse.json({ ok: false, error: "invalid_plan_days" }, { status: 400 });
        }
        const planDays: PlanDays = planDaysRaw;
        const amountNumber = typeof order.amount === "number" ? order.amount : Number(order.amount);
